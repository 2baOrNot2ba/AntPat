...
def readNECout_tvecfuns(filename):
  ...
  #Check to see if mesh wraps around in azimuth. If it does, remove the redundant
  #components. (It may have to been done elsewhere)
  if numpy.fmod(phiMsh[0,0], 2*math.pi) == numpy.fmod(phiMsh[0,-1], 2*math.pi):
...
def _readNECout_FFnextFreq(fp):
  ...
  while True:
    ...
    if not aline or ('DATA CARD' in aline):
      ...
      if ('LINEAR' in aline) or ('RIGHT' in aline) or ('LEFT' in aline):
        theta, phi, GainVert, GainHor, GainTot, PolAx, PolTlt, PolSns, EthetaMag, EthetaPhs, EphiMag, EphiPhs = aline.split()
      else:
        theta, phi, GainVert, GainHor, GainTot, PolAx, PolTlt, EthetaMag, EthetaPhs, EphiMag, EphiPhs = aline.split()
  ...
  EthetaA = numpy.transpose((numpy.array(EthetaMagList)
                          *numpy.exp(1j*numpy.array(EthetaPhsList)*numpy.pi/180)
                          ).reshape(NrPhis, NrThetas))
  EphiA = numpy.transpose((numpy.array(EphiMagList)
                        *numpy.exp(1j*numpy.array(EphiPhsList)*numpy.pi/180)
                        ).reshape(NrPhis, NrThetas))
