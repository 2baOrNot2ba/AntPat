  ...
  for l in range(1,lstop+1):
    ...
    if l==1:
      ...
      for m in range(1,(l-1)+1):
        K1_theta_Lm0pos[:,m]=-np.sqrt((l-1)*(2*l+1)/float((l-m)*(l+m)*(l+1)))*(
             1j*np.sqrt((2*l-1))*x*K1_theta_Lm1pos[:,m]
             -np.sqrt((l-2)*(l-m-1)*(l+m-1)/float((l)*(2*l-3)))*K1_theta_Lm2pos[:,m])
        K1_phi_Lm0pos[:,m]=(-K1_theta_Lm1pos[:,m]
              *np.sqrt((l-1)*(l-m)*(l+m)*(2*l+1)/float((l+1)*(2*l-1)))
              +1j*K1_theta_Lm0pos[:,m]*x*l)/m
      #m==l
      K1_theta_Lm0pos[:,l]=( 1j*K1_theta_Lm1pos[:,l-1]*sintheta*np.exp(1j*phi)
                            *np.sqrt(l*(2*l+1)/float(2*(l-1)*(l+1))) )

    #m==0
    K1_theta_Lm0pos[:,0]=snil
    K1_phi_Lm0pos[:,0]=1j*K1_theta_Lm0pos[:,1]*np.exp(-1j*phi)*sintheta*np.sqrt(l*(l+1))
    #m==l
    #The K1_theta_Lm0pos[:,l] was already set above (since formula not for l==1) 
